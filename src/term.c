 ...
 * @return void
 */
void cursor_move(uint row, uint column) { printf("\033[%u;%uH", row, column); }
...
void cursor_up_down(uint rows, char direction) {
  (direction == 'u')   ? (printf("\033[%uA", rows))
  : (direction == 'd') ? (printf("\033[%uB", rows))
                       : 1;
}
...
void cursor_left_right(uint columns, char direction) {
  (direction == 'l')   ? (printf("\033[%uD", columns))
  : (direction == 'r') ? (printf("\033[%uC", columns))
                       : 1;
}
